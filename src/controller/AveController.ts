...
class AveController extends Ave {
    ...
    public async novo(req: Request, res: Response) {
        try {
            ...
            if (result) {
                ...
                return res.status(400).json('Não foi possivel cadastrar a ave no banco de dados');
            }
        } catch (error) {
            console.log(`Erro ao cadastrar a ave: ${false}`);
            return res.status(400).json('Não foi possivel cadastrar a ave no banco de dados');
        }
    }
    ...
    public async remover(req: Request, res: Response) {
        try {
            ...
            console.log(`Erro ao acessar o modelo: ${error}`);
            return res.status(400).json("Error ao remover ave, consulte os logs no servidor");
        }
    }
    ...
    public async atualizar(req: Request, res: Response): Promise<Response> {
        try {
            ...
            console.log(`Erro ao acessar o modelo: ${error}`);
            return res.status(400).json("Error ao atualizar ave, consulte os logs no servidor");
        }
    }
}
